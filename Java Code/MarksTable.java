   ...
   double sem2;
    ...
    public void actionPerformed(ActionEvent e) {
		...
		try {
           ...
           String   s2="update marks inner join subject on marks.sub_code = subject.sub_code set marks.credit = subject.credit_t + subject.credit_p" ;
           stmt.addBatch(s1);
           stmt.addBatch(s2);
           stmt.executeBatch();
        ...
        if(sem==1) {
        	String s="select * from result where e_no=?";
        	PreparedStatement preparedStatement4 = Main.con.prepareStatement(s);
        	preparedStatement4.setString(1, eno);
        	ResultSet rs=preparedStatement4.executeQuery();
        	
        	if(rs.next()) {
        		//System.out.print(rs.next());
        		
        		 String sql4="update result set sem1=? where e_no=?";
        		 PreparedStatement preparedStatement5 = Main.con.prepareStatement(sql4);	
        		 preparedStatement5.setDouble(1,arr[0]);
                 preparedStatement5.setString(2,eno); 
                 preparedStatement5.executeUpdate();
        	}else{String sql3 = "insert into result(e_no,sem1,sem2,sem3,sem4,sem5,sem6,sem7,sem8,ogpa) values(?,?,?,?,?,?,?,?,?,?)";
        	sgpa[0]=arr[0];		
...
//            String sql2 = "UPDATE RESULT set sem1=?,sem2=?,sem3=?,sem4=?,sem5=?,sem6=?,sem7=?,sem8=? where e_no=? ";
             String sql2 = "UPDATE RESULT set sem"+sem+"=? where e_no=?";
             PreparedStatement preparedStatement2 = Main.con.prepareStatement(sql2);
             preparedStatement2.setDouble(1, arr[0]);
             preparedStatement2.setString(2,eno);
